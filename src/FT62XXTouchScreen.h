...
#include <Wire.h>
  ...
  #define FT62XX_REG_MONITORMODE 0x01    //!< Monitor mode
  ...
  #define FT6206_CTRL_KEEP_AUTO_SWITCH_MONITOR_MODE  0x01  //!< Switching from Active mode to Monitor mode automatically when there is no touching
  ...
  class FT62XXTouchScreen {
    ...
    public:
      ...
      bool begin() {
        bool ok = Wire.begin((int)m_sda, (int)m_scl, 400000U);
        #ifdef TOUCHSCREEN_DEBUG
          Serial.print("SDA=");
          Serial.println(m_sda);
          Serial.print("SCL=");
          Serial.println(m_scl);
          Serial.println("---------------------");
        ...
        // Switch automatically to monitor mode (JKV)
        writeByteToTouch(FT6206_CTRL_REG,FT6206_CTRL_KEEP_AUTO_SWITCH_MONITOR_MODE);
